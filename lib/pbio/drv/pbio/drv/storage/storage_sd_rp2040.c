...
bool pbdrv_storage_sd_is_present(void) {
    if (SDCARD_PIN_CD != 0xFF) {
        return !gpio_get(SDCARD_PIN_CD);  // Active low
    }
...
// ============================================================================
// Pybricks Storage Interface
// ============================================================================

// Simple file-like interface for storing user programs and settings

int pbdrv_storage_read_program(uint8_t *buffer, uint32_t offset, uint32_t size) {
    if (!sd_state.initialized) {
        int err = pbdrv_storage_sd_init();
        if (err != 0) return err;
    }
    
    uint32_t start_sector = STORAGE_PROGRAM_START_SECTOR + (offset / 512);
...
int pbdrv_storage_write_program(const uint8_t *buffer, uint32_t offset, uint32_t size) {
    ...
    if (offset + size > STORAGE_PROGRAM_MAX_SIZE) {
        return -1;
    }
    ...
    while (bytes_written < size) {
        // If not aligned, read-modify-write
