...
#include "../../lib/pbio/drv/motion_dsp/motion_profiler_dsp.h"
#endif
...
void system_test_parse_command(const char *cmd) {
    if (strncmp(cmd, "LED ", 4) == 0) {
	    ...
	    send_response("MOTION TEST", "Complete - check serial");
	#else
	    send_response("ERROR", "Requires Pico 2W");
	#endif
        ...
        send_response("  MOTION TEST    - Run motion profiling tests (multi-axis)\n");
